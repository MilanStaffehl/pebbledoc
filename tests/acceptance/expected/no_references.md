...
There is one sub-package, the `observation` package.
...
A `CelestialObject` tracks basic **positional** information --
...
> [!CAUTION]
>
> Instances are **not** thread-safe; see `kind` mutation in
> `_recompute`.
...
This is a naive placeholder -- see `ObservableMixin`
...
> [!IMPORTANT]
>
> Subclasses **must** implement `observe`; the base class raises
...
An object that is both a `CelestialObject` and
`ObservableMixin`.

> **Hint:**
>
> This class mostly exists to exercise multiple inheritance; prefer
> `Star` or `VariableStar` for real use.
